...
def compute_ortho6d_from_rotation_matrix(poses):
    """
    :param poses: [N, 3, 3]
    :return: [N, 6]
    """
    x, y, z = torch.split(poses, (1, 1, 1), dim=-1)
    x = x.view(-1, 3)
    y = y.view(-1, 3)
...
def compute_rotation_matrix_from_expmap(e):
    """
    :param expmap: [:, 3]
    :return:
    """
    theta = torch.sqrt(e.pow(2).sum(1))[:, None]
    w = torch.cos(0.5 * theta)
    axes = normalize_vector(e)
    # print('axes shape ' + str(axes.size()))
    # print('theta shape ' + str(theta.size()))
    xyz = torch.sin(0.5 * theta) * axes
    q = torch.cat((w, xyz), 1)
    matrix = compute_rotation_matrix_from_quaternion(q)
    return matrix
